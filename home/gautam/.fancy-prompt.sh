...
__powerline() {
    ...
    __git_info() {
        ...
        stats="$(git status --porcelain --branch | grep -o '\[.\+\]$')"
        ...
        if [ -n "$(git status --porcelain)" ]; then
            printf "%s" "${BG_YELLOW}$RESET$FG_BASE02$BG_YELLOW $branch$marks $FG_YELLOW"
        else
            printf "%s" "${BG_MAGENTA}$RESET$FG_BASE02$BG_MAGENTA $branch$marks $RESET$FG_MAGENTA"
        fi
    }
    ...
    ps1() {
        ...
        PS1+="$FG_BASE02$BG_GREEN \\u@\\h $RESET"
        PS1+="$FG_GREEN$BG_BLUE$RESET$FG_BLUE"

        PS1+="$FG_BASE02$BG_BLUE \\W $RESET"

        PS1+="$FG_BLUE$(__git_info)"
        PS1+="$BG_EXIT$RESET"
        PS1+="$BG_EXIT$FG_BASE02 ${PS_SYMBOL} ${RESET}${FG_EXIT}${RESET} "
    }
